...
def question_create():  # Tạo ra câu hỏi gồm ID 3 con vật ngẫu nhiên
    ...
    while len(questionID) < 3:
        questionID.add(random.randint(1, 5))
    return questionID
...
def play_text(text1):
    ...
    output.save("./data/sound/text1.mp3")
    playsound("./data/sound/text1.mp3")
    os.remove("./data/sound/text1.mp3")
...
while True:
    ...
    if marker_IDs is not None:
        marker_IDs = np.array(marker_IDs).reshape(-1, 1)
    ...
    if GameState == 0:
        point = 0  # reset point
        if keyboard.is_pressed('s') or 31 in markerDetected:
            GameState = 1
            play('start')
        timeIdx = 0
        lanchoi = 4
    ...
    # gamestate 1
        ...
        lanchoi -=1
        if (lanchoi > 0):
            # play('lần chơi thứ ' + str(4-lanchoi))
        # Random ID con vật
    ...
    # gamestate 2
        ...
        if frameCount % CHECK_FREQ == 0:
            ...
            if checkStatus == 0:
                if marker_corners:
                    if len(markerDetected) > 3:
                        # Nhiều hơn 3 marker khác nhau đc nhận dạng
                        # playsound
                        play('3Only')
                        checkStatus = 1

                    elif len(markerDetected) == 1 or len(markerDetected) == 2:
                        remindIdx += 30
